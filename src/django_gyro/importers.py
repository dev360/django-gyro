...
class PostgresImporter:
    ...
    def validate_required_columns(self, model: models.Model, csv_headers: List[str]) -> None:
        ...
        for field_name, field in model_fields.items():
            ...
            field_in_csv = (
                field_name in csv_headers or 
                f"{field_name}_id" in csv_headers or
                field_name in field_mapping.values()
            )
    ...
    def validate_row_data(self, model: models.Model, row_data: Dict[str, str]) -> Dict[str, Any]:
        ...
        for field_name, field in model_fields.items():
            ...
            field_in_row = (
                field_name in row_data or 
                f"{field_name}_id" in row_data
            )
        ...
        for csv_column, value in row_data.items():
            ...
            try:
                ...
                raise ValueError(f"Invalid data type for field '{csv_column}': {e}")
    ...
    def import_data(self, model: models.Model, csv_data: List[Dict[str, str]]) -> None:
        ...
        try:
            ...
            if 'UNIQUE constraint' in str(e):
                raise ValueError(f"Unique constraint violation: {e}")
...
class FKDependencyValidator:
    ...
    def detect_cyclical_dependencies(self, importers: List[type]) -> List[List[models.Model]]:
        ...
        def _has_cycle(node: models.Model, visited: Set[models.Model], rec_stack: Set[models.Model], path: List[models.Model]) -> Optional[List[models.Model]]:
            visited.add(node)
            rec_stack.add(node)
            path.append(node)
    ...
    def validate_excluded_fields(self, importer_class: type) -> Dict[str, Any]:
        ...
        return {
            'valid': len(invalid_exclusions) == 0,
                    ...
                    if hasattr(importer_class, 'model') and importer_class.model in cycle:
                        ...
                        for field in importer_class.model._meta.fields:
                            if isinstance(field, models.ForeignKey):
                                ...
                                if (fk_field_name in excluded_fields and 
                                    field.related_model in cycle):
                                    cycle_resolved = True
                                    break
