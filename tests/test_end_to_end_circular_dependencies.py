...
from src.django_gyro.importing import (
    ImportContext, 
...
from gyro_example.models import (
    Tenant, Shop, Customer, Product, Order, OrderItem, CustomerReferral
)
...
class EndToEndCircularDependencyTest(TestCase):
    ...
    def test_loading_order_with_circular_dependencies(self):
        ...
        self.assertLess(customer_idx, referral_idx, 
                       "Customer should load before CustomerReferral")
    ...
    def test_tenant_aware_remapping_with_uuids(self):
        ...
        self.assertEqual(
            all_mappings['Tenant'][self.source_tenant.id], 
            self.target_tenant.id
        )
    ...
    def test_export_with_circular_dependencies(self):
        ...
        with open(customer_csv, 'r') as f:
            content = f.read()
            self.assertIn(str(self.referral.id), content, 
                         "Customer CSV should contain referral ID")
        
        referral_csv = export_dir / "gyro_example_customerreferral.csv"
        with open(referral_csv, 'r') as f:
            content = f.read()
            self.assertIn(str(self.customer1.id), content,
                         "Referral CSV should contain customer ID")
