...
from django_gyro.importing import CircularDependencyResolver, CircularDependency
...
class TestCircularDependencyResolver:
    ...
    def test_prepares_deferred_updates(self):
        ...
        csv_data = {
            'test_asset': [
                {'id': 1, 'name': 'Server', 'risk': 10},
                {'id': 2, 'name': 'Database', 'risk': 20}
            ]
        }
    ...
    def test_executes_deferred_updates_with_remapping(self):
        ...
        updates = [
            {'model': Asset, 'pk': 1, 'field': 'risk', 'value': 10}
        ]
